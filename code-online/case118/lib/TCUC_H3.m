% 编写data based LPF TCUC in hybrid mode
% 包含冗余约束识别
function ot_SCUC = TCUC_H2(mpc,casename,Constraints_Set)
%% Load Parameters
...
try
    ...
    disp('H-LPF! Please import the dir');
    f_this = uigetdir();
    load([f_this,'\',casename,'.mat']);
    m.dir_Xp_H = f_this;
end
...
if Constraints_Set.trn == 1
    st = st + ( -Fmax(ida1,:)*ones(1,T) <= plf(ida1,:) <= Fmax(ida1,:)*ones(1,T) );
    st = st + ( -Fmax(ida2,:)*ones(1,T) <= plt(ida2,:) <= Fmax(ida2,:)*ones(1,T) );
end
