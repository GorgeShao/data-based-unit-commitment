...
%  含冗余识别
function output_SCUC = TCUC_DC2(mpc,Constraints_Set,load_factor)
%% Load Parameters
...
if Constraints_Set.trn == 1
   ST = ST + ( -Fmax(ida,:)*ones(1,T) <= t_flow(ida,:) <= Fmax(ida,:)*ones(1,T) );
end

% if Constraints_Set.trn == 1
%     for i = 1:size(ida,1)
%         if ida(i,2) == 1
%             ST = ST + ( t_flow(ida(i,1),:) <= Fmax(ida(i,1),:)*ones(1,T) );
%         end
%         if ida(i,2) == 2
%             ST = ST + ( -Fmax(ida(i,1),:)*ones(1,T) <= t_flow(ida(i,1),:) );
%         end
%     end
% end
% 爬坡约束
